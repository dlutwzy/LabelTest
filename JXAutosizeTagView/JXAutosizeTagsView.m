...
@interface JXAutosizeTagsView()

@end

@implementation JXAutosizeTagsView
...
- (void)createTagButton{
    ...
    for (int i = 0; i < _tagArray.count; i++) {
        ...
        // 圆角
        btn.layer.cornerRadius = btn.frame.size.height / 4.f;
        ...
        UILabel *closeLabel = [[UILabel alloc] initWithFrame:CGRectMake(frame.size.width - 10, 0, 12, 12)];
        closeLabel.font = [UIFont systemFontOfSize:10.0];
        closeLabel.textColor = [UIColor blackColor];
        closeLabel.text = @"x";
        [btn addSubview:closeLabel];

        //----- 选中事件
        [btn addTarget:self action:@selector(tagTap:) forControlEvents:UIControlEventTouchUpInside];
